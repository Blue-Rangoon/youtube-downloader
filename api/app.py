...
def download():
    ...
    try:
        ydl_opts = {
            ...
            # Use Android client heuristics to sometimes bypass without cookies
            "extractor_args": {"youtube": ["client=android,web"]} 
        }

        # Check for cookies.txt in the project root to authenticate Vercel server 
        cookie_path_root = os.path.join(os.path.dirname(__file__), "..", "cookies.txt")
        cookie_path_local = "cookies.txt"
        
        if os.path.exists(cookie_path_root):
            ydl_opts["cookiefile"] = cookie_path_root
...
if __name__ == '__main__':
    app.run(port=5000, debug=True) #port set to open for localhost